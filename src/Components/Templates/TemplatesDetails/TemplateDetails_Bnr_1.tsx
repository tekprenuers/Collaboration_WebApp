...
const TemplateDetails_Bnr_1: React.FC<TemplateProps> = ({ name, image }) => {
  return (
    <section className="lg:p-[4%]">
      <div className="flex flex-col md:flex-row rounded-lg min-h-screen md:gap-25">
        {/* Left side - Template Image */}
        <div className="flex-1 flex items-center justify-center">
          <img
            src={image}
            alt={name}
            className="rounded-md shadow-md max-w-xl h-auto max-h-screen w-full object-cover bg-amber-600"
          />
        </div>
